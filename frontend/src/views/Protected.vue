<template>
    <div>
        ...
        <div v-if="securedApiCallSuccess">
            <span class="badge bg-success">API call</span> Full response: {{ backendResponse }} <span
            class="badge bg-success">successful</span>
        </div>
        <div v-if="errors">
            <span class="badge bg-warning">API call</span> {{ errors }} <span
            class="badge bg-warning">NOT successful</span>
        </div>
    </div>
...
export default defineComponent({
    ...
    data: (): State => {
        return {
            backendResponse: '', securedApiCallSuccess: false, errors: [],
        }
    }, methods: {
